...
module.exports = flow = function(temporaryFolder) {
    ...
    fs.exists($.temporaryFolder, function(exist) {
        if (!exist) {
    ...
    function shouldMergeChunks(chunkNumber, totalChunk) {
    ...
    function mergeChunks(chunks, writableStream) {
    ...
    $.post = function(req, callback) {
        ...
        if (validation == 'valid') {
            ...
            //var fileStream = fs.createWriteStream(path.resolve($.temporaryFolder, `./${filename}`));
            var numberOfChunks = Math.max(Math.floor(totalSize / (chunkSize * 1.0)), 1);
            ...
            writeStream.on('finish', function() {
                fs.unlink(files[$.fileParameterName].path, function() {
                    $.chunks.push(writeStream);
                    if (shouldMergeChunks($.chunks.length, numberOfChunks)) {
                        //$.write(identifier, fileStream);
    ...
    $.write = function(identifier,fileName, writableStream, options) {
        options = options || {};
        options.end = (typeof options['end'] == 'undefined' ? true : options['end']);
        var testStream="/temp/"+fileName;
